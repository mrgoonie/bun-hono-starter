...
export async function processCashTransaction(userId: string, cashType: CashType, amount: number): Promise<UserBalance> {
	try {
...
export async function processMembership(params: { productId: string; bill: Bill; payment: Payment }) {
	...
	try {
		...
		if (existingUserRole) {
			...
			return await prisma.userRole.update({
				...
				data: {
					endDate: new Date(new Date().getTime() + durationInDays * DEFAULT_DAY_MIN_MILISECOND),
				},
			});
		} else {
		...
		throw new Error(`Process Membership failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
	}
}
