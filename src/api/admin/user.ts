...
export const usersRouter = createTRPCRouter({
	...
	getDetail: adminProcedure.input(z.object({ id: z.string() })).query(async ({ ctx, input }) => {
		const { password, createdAt, updatedAt, ...rest } = await ctx.prisma.user.findUniqueOrThrow({
			where: { id: input.id },
