...
export default async function generateWorkspaceByUser(user: User, input?: GenerateWorkspaceByUserProps) {
	//
	try {
		const name = input?.name || `${user.name}'s Workspace`;
		const description = input?.description || `Amazing ${user.name} Workspace`;
		const isPublic = input?.hasOwnProperty('isPublic') ? toBool(input.isPublic) : false;
		...
		const adminRole = workspace.workspaceRoles.find((role) => role.name === WorkspaceRoleDefault.ADMIN);
